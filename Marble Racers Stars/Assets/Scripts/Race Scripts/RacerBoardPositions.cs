...
#region IRaceSettings Methods
    ...
    public void FillMyMarbles(List<Marble> marblesObteined)
    {
        ...
        board.participantScores = new BoardParticipant[(isManufacturers) ? LeagueManager.LeagueRunning.TeamsNumber : marblesObteined.Count];
        //board.participantScores= new BoardParticipant[marblesObteined.Count];
        board.ResetParticipantSorted();

        for (int i = 0; i < ((isManufacturers)? LeagueManager.LeagueRunning.TeamsNumber: marblesObteined.Count); i++)
